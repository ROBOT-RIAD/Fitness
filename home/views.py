...
from .serializers import DailyMealTodaySerializer, DailyWorkoutTodaySerializer,AIRecommendedDataSerializer,SpanishDailyMealTodaySerializer,SpanishDailyWorkoutTodaySerializer
...
class SpanishTodayDailyDetailsAPIView(APIView):
    permission_classes = [IsAuthenticated]
    @swagger_auto_schema(
        operation_summary="Get DailyMeal & DailyWorkout by date (default: today)",
        tags=["home"],
    ...
    def get(self, request):
        ...
        return Response({
            "date": target_date,
            "daily_meal": SpanishDailyMealTodaySerializer(daily_meal).data if daily_meal else None,
            "daily_workout": SpanishDailyWorkoutTodaySerializer(daily_workout).data if daily_workout else None,
            "AiRecomended":serializer.data,
})
